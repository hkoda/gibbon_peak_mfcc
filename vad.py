...
class VoiceActivityDetector():
    ...
    This code was reused and modifed from https://github.com/marsbroshok/VAD-python, by H. Koda under MIT licences.
    """
    ...
    def _peak_energy_in_band(self,energy_frequencies,start_band,end_band):
        fft_list = []
        for f in energy_frequencies.keys():
            if start_band < f < end_band:
                fft_list.append(energy_frequencies[f])
        peak_energy = max(fft_list)
        return peak_energy
    ...
    def convert_windows_to_readible_labels(self, detected_windows):
        ...
        for window in detected_windows:
            if (window[1]==1.0 and is_speech==0):
                ...
                speech_label['speech_begin_sample'] = window[0]
            if (window[1]==0.0 and is_speech==1):
                is_speech = 0
                speech_time_end = window[0] / self.rate
                speech_label['speech_end'] = speech_time_end
                speech_label['speech_end_sample'] = window[0]
                if speech_time_end - speech_time_start > 0.5: # >0.5 sec required for a phrase
    ...
    def detect_speech(self):
        ...
        sum_voice_energy_s = np.array([])
        peak_energy_s = np.array([])
        ...
        while (sample_start < (len(data) - sample_window)):
            ...
            peak_energy = self._peak_energy_in_band(energy_freq,500,1500)
            sum_full_energy = sum(energy_freq.values())
            speech_ratio = sum_voice_energy/sum_full_energy
            sum_voice_energy_s = np.append(sum_voice_energy_s,sum_voice_energy)
            peak_energy_s = np.append(peak_energy_s,peak_energy)
            # Here I hypothesized that when there is a speech sequence we have ratio of energies more than Threshold
        ...
        return detected_windows, sum_voice_energy_s, peak_energy_s
